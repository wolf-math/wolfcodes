...
A float is a positive or negative number, or zero,  with a foating point attached to the end. If the number doesn't have a floating point then it is a [`int`](./int.md). An operation between a float and an int will result in a float, even if the result is a whole number.
