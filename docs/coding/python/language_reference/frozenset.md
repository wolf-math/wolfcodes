...
A `frozenset` is the immutable version of a [`set`](./set.md). Like sets, frozensets are unordered collections of unique, hashable elements. However, unlike sets, frozensets cannot be modified after creation, which makes them hashable and allows them to be used as dictionary keys or elements in other sets.
