...
A **relative path** is relative to where you currently are. It doesn't start with `/`—instead, it tells you how to get somewhere from your current location.
...
Now let's learn the essential commands for working with files and directories. These three commands—`pwd`, `ls`, and `cd`—are the foundation of command line navigation.
...
Think of `pwd` as asking the computer: "What's my current address?"
